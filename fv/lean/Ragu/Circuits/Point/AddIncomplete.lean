...
def main (idx : ℕ) (input : Var Inputs (F p)) : Circuit (F p) (Var Outputs (F p)) := do
  ...
  let delta ← Element.DivNonzero.generalCircuit idx ⟨y2 - y1, tmp⟩
...
def Assumptions (curveParams : Spec.CurveParams p) (idx : ℕ)
    (input : Inputs (F p)) (data : ProverData (F p)) (hint : ProverHint (F p)) :=
  input.P1.isOnCurve curveParams ∧ input.P2.isOnCurve curveParams ∧
  Element.DivNonzero.GeneralAssumptions idx ⟨input.P2.y - input.P1.y, input.P2.x - input.P1.x⟩ data hint
...
theorem soundness (curveParams : Spec.CurveParams p) (idx : ℕ) :
    GeneralFormalCircuit.Soundness (F p) (elaborated idx) (Spec curveParams) := by
  circuit_proof_start
  simp [circuit_norm,
...
theorem completeness (curveParams : Spec.CurveParams p) (idx : ℕ) :
    GeneralFormalCircuit.Completeness (F p) (elaborated idx) (Assumptions curveParams idx) := by
  circuit_proof_start [
...
def circuit (curveParams : Spec.CurveParams p) (idx : ℕ) :
    GeneralFormalCircuit (F p) Inputs Outputs :=
  {
    elaborated idx with
