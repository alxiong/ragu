...
fn setup_circuit_rx(mut rng: StepRng) -> (Fp, Fp) {
...
fn setup_square_circuit_rx(mut rng: StepRng) -> Fp {
    Fp::random(&mut rng)
}
...
fn rx(witness: (Fp, Fp)) {
    black_box(MySimpleCircuit.rx::<R<5>>(witness, &BENCH_KEY).unwrap());
}
...
fn square_rx_2(witness: Fp) {
    black_box(
        SquareCircuit { times: 2 }
            .rx::<R<13>>(witness, &BENCH_KEY)
            .unwrap(),
    );
}
