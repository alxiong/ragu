...
(Pallas/Vesta) where each curve's scalar field is the other's base field. 
...
hash a commitment $A$ (Vesta point with $\F_q$ coordinates) into the transcript.
...
If we want to operate on the original commitment $A\in\G_1$ (say $s\cdot A$),
...
$A\in\G_1$ and can enforce scalar multiplication natively.
...
The group operations over the original $A, B\in\G_1$ and part 1 of the 
