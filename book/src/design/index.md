...
Ragu's design is informed by practical requirements for its deployment in Zcash as part of [Project Tachyon](https://tachyon.z.cash/). We seek a performance profile that comparable to the existing proofs deployed in the [Orchard protocol](https://zcash.github.io/orchard/) (based on the [`halo2`](github.com/zcash/halo2) crate). We would also prefer compatibility with the [Pasta](https://electriccoin.co/blog/the-pasta-curves-for-halo-2-and-beyond/) elliptic curves (using for signing in hardware wallets, for example) as deployed in Orchard.
